...
const MINIMAX_LOG_DIRS = [
  join(homedir(), '.minimax', 'logs'),
  join(homedir(), '.config', 'minimax', 'logs'),
];
...
async function parseMinimaxJsonl(filepath) {
  const turns = [];
...
export async function scan(db, options = {}) {
  const dirsToScan = options.dirs || MINIMAX_LOG_DIRS;
  ...
  for (const dir of dirsToScan) {
    ...
    if (verbose) console.log(`  [minimax] Scanning ${dir} ...`);
    ...
    for (const file of files.sort((a, b) => a.path.localeCompare(b.path))) {
      ...
      const { sessions, turns, lineCount } = await parseMinimaxJsonl(file.path);
  ...
  if (verbose) {
    console.log(`  [minimax] Done: ${result.new} new, ${result.skipped} skipped, ${result.turns} turns`);
  }
  return result;
}

...
const OPENAI_LOG_DIRS = [
  join(homedir(), '.openai', 'logs'),
  join(homedir(), '.config', 'openai', 'logs'),
];
...
async function parseOpenaiJsonl(filepath) {
  const turns = [];
  ...
  for await (const line of rl) {
    ...
    const timestamp = record.timestamp || record.created_at || record.created || '';
    ...
    if (usage) {
      ...
      cacheCreation = usage.prompt_cache_creation_tokens || 0;
    }

    // OpenAI chat completions format (response object)
    if (!usage && record.response) {
      ...
      reasoningTokens = respUsage.reasoning_tokens || 0;
      cacheRead = respUsage.prompt_tokens_details?.cached_tokens || 0;
    }

    if (inputTokens + outputTokens + reasoningTokens + cacheRead + cacheCreation === 0) continue;

    const messageId = record.id || record.response?.id || '';
...
export async function scan(db, options = {}) {
  const dirsToScan = options.dirs || OPENAI_LOG_DIRS;
  ...
  for (const dir of dirsToScan) {
    ...
    if (verbose) console.log(`  [openai] Scanning ${dir} ...`);
    ...
    for (const file of files.sort((a, b) => a.path.localeCompare(b.path))) {
      ...
      const { sessions, turns, lineCount } = await parseOpenaiJsonl(file.path);
  ...
  if (verbose) {
    console.log(`  [openai] Done: ${result.new} new, ${result.skipped} skipped, ${result.turns} turns`);
  }
  return result;
}
